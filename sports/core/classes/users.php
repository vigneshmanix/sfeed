...
public function score($home_score,$home_name,$opp_score,$opp_name,$admin_id,$gametime)
{	
	$query = $this->db->prepare("INSERT INTO `score`(`Id`, `homescore`,`homename`,`oppscore`,`oppname`,`scoredate`,`gametime`) VALUES ('$admin_id','$home_score','$home_name','$opp_score','$opp_name','$gametime',now())");
	try{
...
public function updatescore($hscore,$hname,$oscore,$oname,$id,$gtime)
{
	$query = $this->db->prepare("UPDATE `score` SET `homescore`='$hscore',`homename`='$hname',`oppscore`='$oscore',`oppname`='$oname',`gametime`='$gtime'  WHERE `score_id`='$id'");
	try{
		return $query->execute();
	}catch(PDOException $e){
		die($e->getMessage());
